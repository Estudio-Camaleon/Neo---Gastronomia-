...
export function AddProductSection({ negocioId }: AddProductSectionProps) {
  ...
  const [productoAEditar, setProductoAEditar] = useState<UnifiedProduct | null>(null);
  ...
  return (
    <div className="space-y-6">
      <div className="admin-card flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <button
            ...
            className="flex-1 md:flex-initial bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 hover:text-gray-900 rounded-lg font-semibold text-sm px-4 py-2.5 flex items-center justify-center gap-2 transition-colors shadow-sm"
          >
      ...
      <div className="admin-card !p-0 overflow-hidden">
        <div className="px-5 py-4 border-b border-[var(--admin-border)] bg-gray-50/50 flex items-center gap-2">
          <Package className="h-4 w-4 text-[var(--admin-text-muted)]" />
          <span className="font-semibold text-gray-700 text-sm">Ítems Activos</span>
        </div>

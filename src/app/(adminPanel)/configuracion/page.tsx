...
export default async function ConfiguracionPage(props: {
  ...
  const { data: negocios } = await supabase
    .from("negocios")
    .select(
      `id, nombre, slug, whatsapp, direccion, localidad, direccion_notas, color_primary, logo_url, logo_scale, logo_posicion, logo_fit, logo_shape, banner_url, banner_posicion, banner_height, banner_scale, mostrar_nombre, horarios, instagram_url, facebook_url, tiktok_url, twitter_url, youtube_url, tripadvisor_url, direcciones, floating_shapes`,
    )
    .eq("user_id", user?.id ?? "")
    .limit(1)

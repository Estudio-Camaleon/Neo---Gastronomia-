...
export default async function PublicMenuPage({ params }: PublicPageProps) {
  ...
  const { data: negocios } = await supabase
    .from("negocios")
    .select(
      "id, nombre, slug, color_primary, banner_url, banner_posicion, banner_height, banner_scale, logo_url, logo_scale, logo_posicion, logo_fit, logo_shape, mostrar_nombre, direccion, localidad, direccion_notas, direcciones, whatsapp, instagram_url, facebook_url, tiktok_url, horarios",
    )
    .eq("slug", slug.toLowerCase())
    .limit(1)
